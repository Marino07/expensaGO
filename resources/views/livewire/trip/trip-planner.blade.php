        ...
        <div class="bg-white rounded-2xl shadow-2xl p-8 max-w-sm mx-auto transform">
            <div class="text-center">
                <div class="relative inline-flex items-center justify-center mb-6">
                    ...
                    <div
                        class="absolute border-4 border-blue-600 rounded-full w-20 h-20 animate-spin border-t-transparent">
                    </div>
                    <!-- Inner ring animation -->
                    <div class="absolute border-4 border-teal-400 rounded-full w-12 h-12 animate-spin border-t-transparent"
        ...
        <div class="max-w-7xl mx-auto">
            ...
            @if (!$planner)
                <div class="text-center py-16 px-4 sm:px-6 lg:px-8">
                    ...
                    <p class="text-2xl text-teal-700 mb-10 max-w-3xl mx-auto">Ready to embark on your next adventure?
                        Generate your personalized travel plan and explore the world with confidence!</p>
                    <button wire:click="generatePlan" x-on:click="isLoading = true" x-bind:disabled="isLoading"
            ...
            @else
                <h2 class="text-4xl md:text-5xl lg:text-4xl font-bold mb-8 text-center">
                    <div class="inline-flex items-center justify-center gap-2 group text-cyan-400">
                        <span
                            class="bg-gradient-to-r from-blue-500 to-cyan-400 text-transparent bg-clip-text transition-all duration-300 ease-in-out transform group-hover:scale-105">
                            Enjoy your trip
                        </span>
                ...
                <div x-data="{ activeDay: {}, isLoading: false }" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    @foreach ($plannerDays as $index => $day)
                        <div class="relative h-[450px] perspective-1000" x-data="{ flipped: false }"
                            x-show="{{ $index < $cardsToShow }}" x-cloak>
                            ...
                            <div class="w-full h-full transition-all duration-500 preserve-3d"
                                ...
                                x-on:click="flipped = !flipped" :style="flipped ? 'transform: rotateY(180deg)' : ''">

                                <!-- Front Content -->
                                <div
                                    class="absolute w-full h-full backface-hidden bg-white rounded-2xl shadow-xl overflow-hidden">
                                    ...
                                    <div class="p-6 flex flex-col h-[calc(100%-76px)]">
                                        <div class="flex-grow">
                                            ...
                                            <div
                                                ...
                                                <div class="flex items-center mb-4">
                                                    ...
                                                    <div class="flex-grow">
                                                        <p class="text-lg font-medium text-blue-800">
                                                            {{ $day->main_attraction['name'] }}</p>
                                                        <div class="flex items-center gap-2 mt-2">
                                                            <span
                                                                class="inline-block bg-teal-100 text-teal-800 text-sm px-3 py-1 rounded-full">
                                                                {{ $day->main_attraction['preference_name'] }}
                                                            </span>
                                                            <span
                                                                class="inline-block {{ $day->main_attraction['price_info'] === 'Free' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-800' }} text-sm px-3 py-1 rounded-full">
                                                ...
                                                <div class="border-t border-blue-100 pt-4">
                                                    <div
                                                        class="flex justify-between items-center text-sm text-blue-700">
                                        ...
                                        <div
                                            class="text-center mt-4 text-gray-500 flex items-center justify-center group cursor-pointer">
                                            <svg class="w-5 h-5 mr-2 transform group-hover:translate-x-1 transition-transform"
                                                fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                    d="M13 7l5 5m0 0l-5 5m5-5H6" />
                                            </svg>
                                            <span class="text-[15px]">Click to see {{ count($day->places_to_visit) }}
                                                more attractions</span>
                                        </div>
                                        ...
                                        <div
                                            class="text-center mt-4 text-blue-500 flex items-center justify-center group cursor-pointer">

                                            <span class="text-[15px]"> 💸 Expense average
                                                {{ round($averageDayCost) }}$ per day </span>
                                        </div>
                                    ...
                                    <div class="p-6 overflow-auto max-h-[350px]">
                                        <ul class="space-y-4">
                                            @foreach ($day->places_to_visit as $place)
                                                <li
                                                    class="bg-gray-50 p-4 rounded-lg transform transition-all duration-300 hover:scale-105 hover:shadow-md">
                                                    <div class="flex items-start gap-4">
                                                        ...
                                                        <div class="flex-grow">
                                                            <div class="flex justify-between items-start">
                                                                <div>
                                                                    <span
                                                                        class="text-gray-800 font-medium block mb-1">{{ $place['name'] ?? 'Unnamed place' }}</span>
                                                                    <div class="flex items-center gap-2">
                                                                        ...
                                                                        <span
                                                                            class="inline-block {{ $place['price_info'] === 'Free' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-800' }} text-xs px-2 py-1 rounded-full">
                                                                            {{ $place['price_info'] }}
                                                                        </span>
                                                                    </div>
                                                                </div>
                            ...
                            <template x-cloak x-if="{{ $index >= $cardsToShow }}">
                                <div class="absolute inset-0 z-10 flex flex-col items-center justify-center">
                                    <svg class="w-12 h-12 text-gray-600 mb-3" fill="currentColor"
                                        viewBox="0 0 24 24">
                                        <path
                                            d="M12 1a5 5 0 0 0-5 5v2H6a2 2 0 0 0-2 2v9a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-9a2 2 0 0 0-2-2h-1V6a5 5 0 0 0-5-5zm3 7H9V6a3 3 0 1 1 6 0z" />
                                    </svg>
