    ...
    <main class="container mx-auto px-4 py-4 relative z-10 flex-grow">
        <div class="flex flex-col items-center mb-6">
            <div class="flex items-center justify-center mb-2">
                <svg class="w-14 h-14 text-red-500 animate-pulse" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                @forelse($savedPlaces as $place)
                    <div class="bg-white rounded-lg shadow-lg overflow-hidden transform transition duration-500 hover:scale-105 hover:shadow-xl relative">
                        ...
                        <button
                            onclick="confirmRemoval({{ $place['id'] }}, 'place')"
                        ...
                        >
                            <svg class="w-6 h-6 text-red-500 hover:scale-110 transition-transform" viewBox="0 0 24 24" fill="currentColor">
                        ...
                        <div class="p-6">
                            <h2 class="text-2xl font-bold mb-2 text-gray-800">{{ $place['name'] }}</h2>
                            <p class="text-gray-600 mb-4">{{ $place['location'] }}</p>
                            <span class="inline-block bg-green-100 text-green-800 rounded-full px-3 py-1 text-sm font-semibold">{{ $place['category'] }}</span>
                        </div>
                    </div>
                @empty
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                @forelse($savedEvents as $event)
                    <div class="bg-white rounded-lg shadow-lg overflow-hidden transform transition duration-500 hover:scale-105 hover:shadow-xl relative">
                        <!-- Remove heart button - ažurirani kod unutar card elementa -->
                        <button
                            onclick="confirmRemoval({{ $event['id'] }}, 'event')"
                            class="absolute top-4 right-4 z-10 bg-white bg-opacity-75 p-2 rounded-full shadow-lg hover:bg-opacity-100 transition-all duration-300"
                        >
                            <svg class="w-6 h-6 text-red-500 hover:scale-110 transition-transform" viewBox="0 0 24 24" fill="currentColor">
                                <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/>
